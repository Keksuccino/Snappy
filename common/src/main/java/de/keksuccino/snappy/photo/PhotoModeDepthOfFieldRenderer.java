...
final class PhotoModeDepthOfFieldRenderer {
    ...
    private static final Identifier DOF_PIPELINE_ID = Identifier.fromNamespaceAndPath(Snappy.MOD_ID, "pipeline/photo_depth_of_field");
    private static final Identifier RESOLVE_PIPELINE_ID = Identifier.fromNamespaceAndPath(Snappy.MOD_ID, "pipeline/photo_depth_of_field_resolve");
    ...
    static void process(
        ...
        ResourceHandle<RenderTarget> resolveTarget = frame.createInternal(
                "snappy depth of field resolve",
                new RenderTargetDescriptor(mainRenderTarget.width, mainRenderTarget.height, false, CLEAR_COLOR, GpuFormat.RGBA8_UNORM)
        );

        FramePass dofPass = frame.addPass("snappy_depth_of_field");
        dofPass.reads(main);
        ...
        FramePass resolvePass = frame.addPass("snappy_depth_of_field_resolve");
        ...
        FramePass copyPass = frame.addPass("snappy_depth_of_field_copy");
    ...
    private static final class Resources implements AutoCloseable {
        ...
        private final ProjectionMatrixBuffer postProjectionMatrixBuffer = new ProjectionMatrixBuffer("snappy photo dof");
        private final MappableRingBuffer samplerInfoBuffer = new MappableRingBuffer(() -> "Snappy photo DOF SamplerInfo", 130, SAMPLER_INFO_SIZE);
        private final MappableRingBuffer dofConfigBuffer = new MappableRingBuffer(() -> "Snappy photo DOF Config", 130, DOF_CONFIG_SIZE);
