...
final class PhotoModeStylizeRenderer {
    ...
    private static final Identifier STYLIZE_PIPELINE_ID = Identifier.fromNamespaceAndPath(Panoramica.MOD_ID, "pipeline/photo_stylize");
    private static final Identifier COPY_PIPELINE_ID = Identifier.fromNamespaceAndPath(Panoramica.MOD_ID, "pipeline/photo_stylize_copy");
    private static final Identifier STYLIZE_SHADER_ID = Identifier.fromNamespaceAndPath(Panoramica.MOD_ID, "post/stylize");
    ...
    static void process(
        ...
        FramePass stylizePass = frame.addPass("panoramica_stylize");
        stylizePass.reads(main);
        ResourceHandle<RenderTarget> stylizedOutput = stylizePass.readsAndWrites(stylizedTarget);
        stylizePass.executes(() -> drawStylizePass(renderResources, main.get(), stylizedOutput.get()));

        FramePass copyPass = frame.addPass("panoramica_stylize_copy");
    ...
    private static boolean ensurePipelinesAvailable() {
        return RenderSystem.getDevice().precompilePipeline(STYLIZE_PIPELINE).isValid()
    ...
    private static void writeUniforms(
        ...
        try (GpuBufferSlice.MappedView view = renderResources.stylizeConfigBuffer.currentBuffer().map(false, true)) {
            Std140Builder builder = Std140Builder.intoBuffer(view.data());
            builder.putVec4(session.stylizePreset().shaderIndex(), 0.0F, 0.0F, 0.0F);
        }
    }
    ...
    private static void drawStylizePass(@NotNull Resources renderResources, @NotNull RenderTarget source, @NotNull RenderTarget output) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(
                () -> "Panoramica photo stylize",
                outputColor,
                Optional.empty(),
                null,
                OptionalDouble.empty()
        )) {
            renderPass.setPipeline(STYLIZE_PIPELINE);
            RenderSystem.bindDefaultUniforms(renderPass);
            renderPass.setUniform("SamplerInfo", renderResources.samplerInfoBuffer.currentBuffer());
            renderPass.setUniform("StylizeConfig", renderResources.stylizeConfigBuffer.currentBuffer());
    ...
    private static final class Resources implements AutoCloseable {
        ...
        private final ProjectionMatrixBuffer postProjectionMatrixBuffer = new ProjectionMatrixBuffer("panoramica photo stylize");
        private final MappableRingBuffer samplerInfoBuffer = new MappableRingBuffer(() -> "Panoramica photo stylize SamplerInfo", 130, SAMPLER_INFO_SIZE);
        private final MappableRingBuffer stylizeConfigBuffer = new MappableRingBuffer(() -> "Panoramica photo stylize Config", 130, STYLIZE_CONFIG_SIZE);
