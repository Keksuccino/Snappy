        ...
        future[0] = EXECUTOR.scheduleAtFixedRate(() -> {
            final Runnable r = () -> {
                try {
                    ...
                    LOGGER.error("[PANORAMICA] Error while trying to execute scheduled (fixed rate) task!", ex);
                }
            };
        ...
        future[0] = EXECUTOR.schedule(() -> {
            final Runnable r = () -> {
                try {
                    ...
                    LOGGER.error("[PANORAMICA] Error while trying to execute scheduled task!", ex);
                }
            };
            if (executeInMainThread) {
                MainThreadTaskExecutor.executeInMainThread(r, MainThreadTaskExecutor.ExecuteTiming.POST_CLIENT_TICK);
        ...
        EXECUTOR.execute(() -> {
            final Runnable r = () -> {
                try {
                    ...
                    LOGGER.error("[PANORAMICA] Error while trying to execute task!", ex);
                }
            };
            if (executeInMainThread) {
                MainThreadTaskExecutor.executeInMainThread(r, MainThreadTaskExecutor.ExecuteTiming.POST_CLIENT_TICK);
